...
class UnitPemeriksaanController extends \yii\web\Controller
{
    ...
    public function actionUnitPemeriksaan($id = null)
    {
        ...
        if ($id != null) {
            ...
            $anamnesis = Anamnesis::find()->where(['nomor_rekam_medik' => $modelDataLayanan->no_rekam_medik])->andWhere(['no_daftar'=>$modelDataLayanan->no_registrasi])->one();
			// var_dump($modelDataLayanan);
			// exit;
            $jenis_pekerjaan = new JenisPekerjaan();
            $master_pemeriksaan_fisik = MasterPemeriksaanFisik::find()->where(['no_rekam_medik' => $modelDataLayanan->no_rekam_medik])->andWhere(['no_daftar'=>$modelDataLayanan->no_registrasi])->one();
...
// exit;
        if ($anamnesis != null) {
							// $anamensis->no_daftar = $p['Anamnesis']['no_daftar'];
    ...
    public function actionSavePemeriksaanFisik()
    {
        ...
        $modelDataLayanan = DataLayanan::find()->where(['no_rekam_medik' => $no_rekam_medik])->orderBy('id_data_pelayanan DESC')->limit(1)->one();
        ...
        if ($master_pemeriksaan_fisik != null) {
            if ($master_pemeriksaan_fisik->load(Yii::$app->request->post())) {
                ...
                if ($master_pemeriksaan_fisik->save(false)) {
                    return [
                        's' => true,
                        'e' => null
                    ];
